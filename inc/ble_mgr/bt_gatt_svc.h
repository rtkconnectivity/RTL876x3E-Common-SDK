...
/**
 * @brief GATT service pending packet number.
 */
typedef struct
{
    uint8_t notify_num;
    uint8_t ind_num;
} T_GATT_SVC_PENDING_NUM;
 ...
 * @param[in]  p_srv_ext_send_data_cb Callback functions for service to handle the result of send notification
                                      or indication.
 *  -    NULL    Application needs to handle @ref PROFILE_EVT_SEND_DATA_COMPLETE directly.
 ...
 *  -    0       Application shall call @ref server_register_app_cb and @ref server_init to register callback
                   and set service number.
 *  -    Others  Application does not need to call @ref server_register_app_cb and @ref server_init.
...
/**
 * @brief    Set the Maximum pending packets in the Bluetooth GATT service module.
 * \xrefitem Experimental_Added_API_2_13_0_0 "Experimental Added Since 2.13.0.0" "Experimental Added API"
 * @param[in] num  Pending packet number @ref T_GATT_SVC_PENDING_NUM.
 *                 The default number for notification is 10. The default number for indication is 5.
 *
 * <b>Example usage</b>
 * \code{.c}
