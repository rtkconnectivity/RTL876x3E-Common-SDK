 ...
 */
/**@{*/
#define GAP_QOS_NO_TRAFFIC              0x00 /**< No traffic. */
#define GAP_QOS_BEST_EFFORT             0x01 /**< Best effort (default). */
#define GAP_QOS_GUARANTEED              0x02 /**< Guaranteed. */
/**@}*/
...
typedef enum
{
    GAP_BR_PARAM_BT_MODE,                    /**< Bluetooth mode. Write only. Parameter type is uint8_t. Valid values are \ref T_GAP_BT_MODE. Default value is \ref GAP_BT_MODE_21ENABLED. */
    GAP_BR_PARAM_NAME,                       /**< BR/EDR name. Write only. Name string length is \ref GAP_DEVICE_NAME_LEN. */
    GAP_BR_PARAM_COD,                        /**< Class of device. Write only. Parameter type is uint32_t. Default value is 0x000900. */
    GAP_BR_PARAM_SUPV_TOUT,                  /**< BR/EDR link supervision timeout value. Write only. Parameter type is uint16_t. Default value is 0x7D00. */
    GAP_BR_PARAM_LINK_POLICY,                /**< Default link policy to set after link established. Write only. Parameter type is uint8_t. Allowed values are combinations of \ref GAP_LINK_POLICY. Default value is \ref GAP_LINK_POLICY_ROLE_SWITCH | \ref GAP_LINK_POLICY_SNIFF_MODE. */
    GAP_BR_PARAM_RADIO_MODE,                 /**< BR/EDR radio mode. Write only. Parameter type is uint8_t. Valid parameters are \ref T_GAP_RADIO_MODE. Default value is \ref GAP_RADIO_MODE_NONE_DISCOVERABLE. */
    GAP_BR_PARAM_LIMIT_DISCOV,               /**< Limited discoverable mode. Write only. Parameter type is bool. Default value is false. */
    GAP_BR_PARAM_LIMIT_DISCOV_TOUT,          /**< Limited discoverable mode timeout value in seconds. Only valid when limited discoverable mode is enabled. Write only. Parameter type is uint8_t. Default value is 60. */
    GAP_BR_PARAM_PAGE_SCAN_TYPE,             /**< BR/EDR page scan type. Write only. Parameter type is uint8_t. Valid values are \ref T_GAP_PAGE_SCAN_TYPE. Default value is \ref GAP_PAGE_SCAN_TYPE_STANDARD. */
    GAP_BR_PARAM_PAGE_SCAN_INTERVAL,         /**< BR/EDR page scan interval. Write only. Parameter type is uint16_t. Default value is 0x800. */
    GAP_BR_PARAM_PAGE_SCAN_WINDOW,           /**< BR/EDR page scan window. Write only. Parameter type is uint16_t. Default value is 0x12. */
    GAP_BR_PARAM_PAGE_TIMEOUT,               /**< BR/EDR page timeout value. Write only. Parameter type is uint16_t. Default value is 0x2000. */
    GAP_BR_PARAM_INQUIRY_SCAN_TYPE,          /**< BR/EDR inquiry scan type. Write only. Parameter type is uint8_t. Valid values are \ref T_GAP_INQUIRY_SCAN_TYPE. Default value is \ref GAP_INQUIRY_SCAN_TYPE_STANDARD. */
    GAP_BR_PARAM_INQUIRY_SCAN_INTERVAL,      /**< BR/EDR inquiry scan interval. Write only. Parameter type is uint16_t. Default value is 0x1000. */
    GAP_BR_PARAM_INQUIRY_SCAN_WINDOW,        /**< BR/EDR inquiry scan window. Write only. Parameter type is uint16_t. Default value is 0x12. */
    GAP_BR_PARAM_INQUIRY_MODE,               /**< BR/EDR inquiry mode. Write only. Parameter type is uint8_t. Valid values are \ref T_GAP_INQUIRY_MODE. Default value is \ref GAP_INQUIRY_MODE_EXTENDED_RESULT. */
    GAP_BR_PARAM_DATA_QUOTA,                 /**< Max number of BR/EDR data to send without response. Read only. Parameter type is uint8_t. */
    GAP_BR_PARAM_AUTO_ACCEPT_ACL,            /**< Bluetooth stack accept ACL connection request automatically. Write only. Parameter type is bool. default value is true. */
    GAP_BR_PARAM_PAGE_SCAN_REPETITION_MODE,  /**< BR/EDR page scan repetition mode. Write only. Parameter type is uint8_t. Valid values are \ref T_GAP_PAGE_SCAN_REPETITION_MODE. Default value is \ref GAP_PAGE_SCAN_REPETITION_R2. */
} T_GAP_BR_PARAM_TYPE;
...
typedef struct
{
    ...
    uint8_t   eir_len;                    /**< Extended inquiry response data length. */
    uint8_t  *p_eir;                      /**< The buffer address of extended inquiry response data. */
