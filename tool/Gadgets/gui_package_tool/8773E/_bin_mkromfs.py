...
parser.add_argument('--ictype', default='8763E', help='output bin for IC type.')
...
def get_userdata_flash_addr(ictype):
    cwd = os.getcwd()
    if ictype == '8763E':
        relative_addr = "../../../../../bin/rtl87x3e/flash_map_config/16M/flash_16M/flash_map.h"
    elif ictype == '87x3D':
        relative_addr = "../../../../../bin/rtl87x3d/flash_map_config/flash_cs_16M/flash_map.h"
    elif ictype == '8773E':
        relative_addr = "../../../../../bin/rtl87x3ep/flash_map_config/16M/flash_16M/flash_map.h"
    else:
        return 0
    file_addr = os.path.join(cwd, relative_addr)
    flash_map = open(file_addr, "r")
    lines = flash_map.readlines()

    keyword = "USER_DATA1_ADDR"
    pattern = r'#define\s+USER_DATA1_ADDR\s+(.*)'
    for line in lines:
        if keyword in line:
            macro = re.findall(pattern, line)
            break
    str = macro[0]
    if ictype == '8763E':
        return int(str[2:], 16)+0x400
    elif ictype == '87x3D':
        return int(str[2:], 16)+0x1000
    elif ictype == '8773E':
        return int(str[2:], 16)+0x400
...
if __name__ == '__main__':
    ...
    if args.ictype:
        address = get_userdata_flash_addr(args.ictype)
    print('userdata address:', hex(address))

    file_addr = ''
    if args.binary: 
        data, file_addr = get_bin_data(tree, address)
    else:
        data = get_c_data(tree)

    print('file_addr:', file_addr)
    ...
    output.write(data)
    with open(os.path.join(os.path.dirname(os.getcwd()), 'resource.h'), 'w') as f:
        f.write(file_addr)
