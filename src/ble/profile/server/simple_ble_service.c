...
static P_FUN_EXT_SERVER_GENERAL_CB pfn_simp_ble_service_cb = NULL;
...
*/
T_APP_RESULT  simp_ble_service_attr_read_cb(uint16_t conn_handle, uint16_t cid,
...
void simple_write_post_callback(uint16_t conn_handle, uint16_t cid, T_SERVER_ID service_id,
                                uint16_t attrib_index,
                                uint16_t length, uint8_t *p_value)
{
    APP_PRINT_INFO5("simple_write_post_callback: conn_handle %d, cid %d,  service_id %d, attrib_index 0x%x, length %d",
 ...
 * @return Profile procedure result
*/
T_APP_RESULT simp_ble_service_attr_write_cb(uint16_t conn_handle, uint16_t cid,
                                            T_SERVER_ID service_id,
                                            ...
                                            P_FUN_EXT_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    TSIMP_CALLBACK_DATA callback_data;
    T_APP_RESULT  cause = APP_RESULT_SUCCESS;
    APP_PRINT_INFO3("simp_ble_service_attr_write_cb write_type = 0x%x, conn_handle 0x%x, cid %d",
                                     ...
                                     uint16_t length)
{
    ...
    return gatt_svc_send_data(conn_handle, cid, service_id, SIMPLE_BLE_SERVICE_CHAR_V3_NOTIFY_INDEX,
                                       ...
                                       void *p_value, uint16_t length)
{
    ...
    return gatt_svc_send_data(conn_handle, cid, service_id, SIMPLE_BLE_SERVICE_CHAR_V4_INDICATE_INDEX,
                              p_value, length, GATT_PDU_TYPE_ANY);
}
 ...
 * @return None
*/
void simp_ble_service_cccd_update_cb(uint16_t conn_handle, uint16_t cid, T_SERVER_ID service_id,
...
*/
const T_FUN_GATT_EXT_SERVICE_CBS simp_ble_service_cbs =
{
...
{
    if (false == gatt_svc_add(&simp_service_id,
                              (uint8_t *)simple_ble_service_tbl,
                              sizeof(simple_ble_service_tbl),
                              &simp_ble_service_cbs, NULL))
    {
    ...
    pfn_simp_ble_service_cb = (P_FUN_EXT_SERVER_GENERAL_CB)p_func;
    return simp_service_id;
}
