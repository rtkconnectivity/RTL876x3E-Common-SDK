...
static P_FUN_EXT_SERVER_GENERAL_CB  pfn_bas_cb = NULL;
...
{
    return gatt_svc_send_data(le_get_conn_handle(conn_id), L2C_FIXED_CID_ATT, service_id,
                              GATT_SVC_BAS_BATTERY_LEVEL_INDEX, &battery_level,
                                          ...
                                          uint8_t battery_level)
{
    if (bas_read_battery_level_pending == true)
    {
        ...
        return gatt_svc_read_confirm(le_get_conn_handle(conn_id), L2C_FIXED_CID_ATT, service_id,
                                     GATT_SVC_BAS_BATTERY_LEVEL_INDEX, &battery_level,
                                     sizeof(battery_level), sizeof(battery_level), APP_RESULT_SUCCESS);
    }
    else
    {
        return false;
