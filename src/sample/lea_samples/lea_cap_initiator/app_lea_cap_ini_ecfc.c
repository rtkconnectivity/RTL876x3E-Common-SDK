...
bool app_lea_ecfc_conn(uint8_t conn_id)
{
    if (APP_LEA_EATT_CID_NUM != 0)
    {
        uint8_t bd_addr[6];
        uint8_t bd_type;
        if (le_get_conn_addr(conn_id, bd_addr, &bd_type))
        {
            if (gap_ecfc_send_conn_req(PSM_EATT, app_ecfc_le_proto_id,
                                       app_ecfc_local_mtu, bd_addr, bd_type, APP_LEA_EATT_CID_NUM))
            {
                return true;
            }
        }
