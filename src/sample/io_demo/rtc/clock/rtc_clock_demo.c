...
#define SYSTEM_ORIGIN_DAY_OF_WEEK   (WEEK_Sat)  //2000-01-01 is sat
...
typedef struct
{
    ...
    volatile T_CLOCK_UTC_TIME             global_time;
...
static void rtc_clock_system_clock_init(uint32_t second)
{
    ...
    RTC_MaskINTConfig(RTC_INT_CMP0 | RTC_CMP0_WK_INT, ENABLE);
    RTC_CompINTConfig(RTC_INT_CMP0 | RTC_CMP0_WK_INT, DISABLE);
...
static void rtc_handler(void)
{
    ...
    if (RTC_GetINTStatus(RTC_INT_CMP0) == SET)
    {
        ...
        RTC_ClearCompWkINT(ClOCK_RTC_COMPARATOR);
    }
