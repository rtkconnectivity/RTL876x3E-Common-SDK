...
#include "app_bt_map_demo_gap.h"
#include "app_bt_map_demo.h"

/** @defgroup  MAP_DEMO_MAIN MAP Demo Main
    * @brief Main file to initialize hardware and BT stack and start task scheduling
    * @{
    */

#define MAP_DEMO_UART_TX          P3_1
#define MAP_DEMO_UART_RX          P3_0

#define MAP_DEMO_DEFAULT_PAGESCAN_WINDOW             0x12
#define MAP_DEMO_DEFAULT_PAGESCAN_INTERVAL           0x800 //0x800
#define MAP_DEMO_DEFAULT_PAGE_TIMEOUT                0x4000
#define MAP_DEMO_DEFAULT_SUPVISIONTIMEOUT            0x1f40 //0x7D00
#define MAP_DEMO_DEFAULT_INQUIRYSCAN_WINDOW          0x12
#define MAP_DEMO_DEFAULT_INQUIRYSCAN_INTERVAL        0x800 //0x1000

#define MAP_DEMO_MAX_NUMBER_OF_GAP_MESSAGE     0x20      //!<  GAP message queue size
#define MAP_DEMO_MAX_NUMBER_OF_IO_MESSAGE      0x20      //!<  IO message queue size
#define MAP_DEMO_MAX_NUMBER_OF_EVENT_MESSAGE   (MAP_DEMO_MAX_NUMBER_OF_GAP_MESSAGE + MAP_DEMO_MAX_NUMBER_OF_IO_MESSAGE)    //!< Event message queue size

#define CONSOLE_UART_RX_BUFFER_SIZE         300

static uint8_t bd_addr_dut[6]  = {0x13, 0x22, 0x56, 0x77, 0x88, 0x66};

void *map_demo_evt_queue_handle;  //!< Event queue handle
void *map_demo_io_queue_handle;   //!< IO queue handle
...
static void app_bt_gap_init(void)
{
    ...
    uint16_t supervision_timeout = MAP_DEMO_DEFAULT_SUPVISIONTIMEOUT;
    ...
    uint16_t pagescan_interval = MAP_DEMO_DEFAULT_PAGESCAN_INTERVAL;
    uint16_t pagescan_window = MAP_DEMO_DEFAULT_PAGESCAN_WINDOW;
    uint16_t page_timeout = MAP_DEMO_DEFAULT_PAGE_TIMEOUT;

    uint8_t inquiryscan_type = GAP_INQUIRY_SCAN_TYPE_STANDARD;
    uint16_t inquiryscan_window = MAP_DEMO_DEFAULT_INQUIRYSCAN_WINDOW;
    uint16_t inquiryscan_interval = MAP_DEMO_DEFAULT_INQUIRYSCAN_INTERVAL;
    ...
    gap_set_bd_addr(bd_addr_dut);
    gap_br_set_param(GAP_BR_PARAM_NAME, GAP_DEVICE_NAME_LEN, "map demo dut");
}
...
{
    /* System Manager */
    sys_mgr_init(map_demo_evt_queue_handle);
...
{
    Pinmux_Config(MAP_DEMO_UART_TX, UART0_TX);
    Pad_Config(MAP_DEMO_UART_TX,
               PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);

    Pinmux_Config(MAP_DEMO_UART_RX, UART0_RX);
    Pad_Config(MAP_DEMO_UART_RX,
               PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);
}
...
static void driver_init(void)
{
    ...
    console_uart_config.uart_rx_pinmux = MAP_DEMO_UART_RX;
    console_uart_config.uart_tx_pinmux = MAP_DEMO_UART_TX;
    console_uart_config.rx_wake_up_pinmux = MAP_DEMO_UART_RX;
 ...
 * @return       void
 */
void map_demo_main_task(void *p_param)
{
    uint8_t event;

    gap_start_bt_stack(map_demo_evt_queue_handle, map_demo_io_queue_handle,
                       MAP_DEMO_MAX_NUMBER_OF_GAP_MESSAGE);

    while (true)
    {
        if (os_msg_recv(map_demo_evt_queue_handle, &event, 0xFFFFFFFF) == true)
        {
            if (EVENT_GROUP(event) == EVENT_GROUP_IO)
            {
                T_IO_MSG io_msg;

                if (os_msg_recv(map_demo_io_queue_handle, &io_msg, 0) == true)
                {
 ...
 * @note     There is only one task in MAP Demo APP, thus only one APP task is init here
 * @return   void
 */
void task_init(void)
{
    ...
    os_msg_queue_create(&map_demo_io_queue_handle, "ioQ", MAP_DEMO_MAX_NUMBER_OF_IO_MESSAGE,
                        sizeof(T_IO_MSG));
    os_msg_queue_create(&map_demo_evt_queue_handle, "evtQ", MAP_DEMO_MAX_NUMBER_OF_EVENT_MESSAGE,
                        sizeof(unsigned char));

    os_task_create(&app_task_handle, "map_demo_main_task", map_demo_main_task, NULL, 1024 * 3, 1);
}
