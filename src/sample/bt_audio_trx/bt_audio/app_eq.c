                                ...
                                uint16_t eq_len)
{
    ...
    else if (app_lea_uca_get_eq_instance() != NULL)
    {
        eq_set(app_lea_uca_get_eq_instance(), eq_buf, eq_len);
    }
}

static void app_eq_media_eq_enable(T_EQ_ENABLE_INFO *enable_info)
{
...
void app_eq_enable_info_get(uint8_t eq_mode, T_EQ_ENABLE_INFO *enable_info)
{
    ...
    if (app_db.sw_eq_type == SPK_SW_EQ)
    {
        ...
        if (eq_mode == VOICE_SPK_MODE)
        {
            ...
            enable_info->instance = link->eq.voice_spk_eq_instance;
        }
        else
#endif
        {
            link = &app_db.br_link[app_a2dp_get_active_idx()];
            enable_info->instance = link->eq.instance;
            enable_info->is_enable = link->eq.audio_eq_enabled;
        }
    ...
    else
    {
        ...
        {
            link = &app_db.br_link[app_hfp_get_active_idx()];
            enable_info->instance = link->eq.voice_mic_eq_instance;
            enable_info->idx = app_cfg_nv.eq_idx;
        }
        else
#endif
                                        ...
                                        uint8_t sample_rate)
{
...
#endif
    {
        ...
        if (eq_type == MIC_SW_EQ)
        {
            offset += (app_eq_cfg.user_eq_spk_eq_num * eq_size);
        }
    }
...
{
    uint8_t new_spk_user_eq_num = app_eq_cfg.user_eq_spk_eq_num;
    uint8_t new_mic_user_eq_num = app_eq_cfg.user_eq_mic_eq_num;
...
void app_eq_check_user_eq(void)
{
    ...
    if (eq_utils_load_eq_from_ftl(0, (uint8_t *)&eq_header, sizeof(T_EQ_USER_EQ_HEADER)) == 0)
    {
        ...
        {
            if (eq_header.spk_user_eq_num != app_eq_cfg.user_eq_spk_eq_num ||
                eq_header.mic_user_eq_num != app_eq_cfg.user_eq_mic_eq_num)
            {
                app_eq_adjust_user_eq(eq_header.spk_user_eq_num, eq_header.mic_user_eq_num);

                eq_header.spk_user_eq_num = app_eq_cfg.user_eq_spk_eq_num;
                eq_header.mic_user_eq_num = app_eq_cfg.user_eq_mic_eq_num;
...
bool app_eq_is_valid_user_eq_index(T_EQ_TYPE eq_type, uint8_t mode, uint8_t index)
{
    ...
    user_eq_num = (eq_type == SPK_SW_EQ) ? app_eq_cfg.user_eq_spk_eq_num :
                  app_eq_cfg.user_eq_mic_eq_num;
...
bool app_eq_reset_user_eq(T_EQ_TYPE eq_type, uint8_t mode, uint8_t index)
{
    ...
    offset = app_eq_get_user_eq_info_offset(eq_type, mode, index, app_eq_cfg.user_eq_spk_eq_num);
...
void app_eq_reset_all_user_eq(void)
{
    ...
    for (eq_index = 0; eq_index < app_eq_cfg.user_eq_spk_eq_num; eq_index++)
    {
        app_eq_reset_user_eq(SPK_SW_EQ, EQ_MODE_NULL, eq_index);
    }

#if F_APP_APT_SUPPORT
    for (eq_index = 0; eq_index < app_eq_cfg.user_eq_mic_eq_num; eq_index++)
    {
...
#else
    spk_user_eq_idx_waiting_sync = app_eq_set_bits(app_eq_cfg.user_eq_spk_eq_num);
#endif

#if F_APP_APT_SUPPORT
    mic_user_eq_idx_waiting_sync = 0;

    if (app_audio_cfg.normal_apt_support && !app_eq_cfg.rws_apt_eq_adjust_separate)
    {
        if (eq_utils_num_get(MIC_SW_EQ, APT_MODE) != 0)
        {
            mic_user_eq_idx_waiting_sync = app_eq_set_bits(app_eq_cfg.user_eq_mic_eq_num);
        }
    }
#endif
...
uint32_t app_eq_sample_rate_get(void)
{
    ...
    sample_rate = link->a2dp.codec.sampling_frequency;

    if (link->a2dp.codec.sampling_frequency == SAMPLE_RATE_44K)
    {
        sample_rate = AUDIO_EQ_SAMPLE_RATE_44_1KHZ;
    }
    else if (link->a2dp.codec.sampling_frequency == SAMPLE_RATE_48K)
    {
        sample_rate = AUDIO_EQ_SAMPLE_RATE_48KHZ;
    }
    else
    ...
    APP_PRINT_TRACE2("app_eq_sample_rate_get: codec_type 0x%x, sample_rate %d",
                     link->a2dp.codec.type, link->a2dp.codec.sampling_frequency);
...
bool app_eq_get_link_info(uint8_t cmd_path, uint8_t app_idx, T_AUDIO_EQ_REPORT_DATA *eq_report_data)
{
    ...
    if (cmd_path == CMD_PATH_SPP || cmd_path == CMD_PATH_IAP || cmd_path == CMD_PATH_GATT_OVER_BREDR)
    {
        T_APP_BR_LINK *p_br_link = (T_APP_BR_LINK *) &app_db.br_link[app_idx];

        if (p_br_link)
        {
            eq_report_data->eq_info = &p_br_link->eq.audio_get_eq_info;
...
bool app_eq_report_eq_frame(T_AUDIO_EQ_REPORT_DATA *eq_report_data)
{
    ...
    if (eq_frame_type == AUDIO_EQ_FRAME_START || eq_frame_type == AUDIO_EQ_FRAME_CONTINUE)
    {
...
void app_eq_play_apt_eq_tone(void)
{
    ...
    app_audio_tone_type_play((T_APP_AUDIO_TONE_TYPE)(TONE_FINDMY_SOUND + eq_index),
                             false, false);
#else
...
#if F_APP_USER_EQ_SUPPORT
            /* EQ mapping */
            buf[1] = app_eq_cfg.user_eq_spk_eq_num;
            buf[2] = app_eq_cfg.user_eq_mic_eq_num;
#endif
    ...
    case CMD_AUDIO_EQ_PARAM_SET:
        {
            ...
            if (cmd_path == CMD_PATH_SPP || cmd_path == CMD_PATH_IAP || cmd_path == CMD_PATH_GATT_OVER_BREDR)
            {
                p_audio_eq_info = &app_db.br_link[app_idx].eq.audio_set_eq_info;
            }
            else if (cmd_path == CMD_PATH_LE)
            {
