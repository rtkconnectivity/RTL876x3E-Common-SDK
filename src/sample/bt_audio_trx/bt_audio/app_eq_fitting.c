...
static void app_eq_fitting_start_playback(void)
{
    ...
    if (p_link && p_link->a2dp.track_handle && p_link->a2dp.is_streaming)
    {
...
static uint8_t app_eq_fitting_write_hw_eq(uint8_t *data, uint32_t len)
{
    ...
    flash_tem_buf = (uint8_t *)malloc(FLASH_NOR_SECTOR_SIZE);
    if (!flash_tem_buf)
    {
...
static bool app_eq_fitting_is_streaming(uint8_t stream_type)
{
    ...
    {
        track = app_db.br_link[app_a2dp_get_active_idx()].a2dp.track_handle;
    }
    else if (stream_type == AUDIO_STREAM_TYPE_VOICE)
    {
        track = app_db.br_link[app_hfp_get_active_idx()].sco.track_handle;
    }
...
static uint8_t app_eq_fitting_get_io_idx(uint8_t io_type)
{
    ...
    switch (io_type)
    {
    case AUDIO_ROUTE_LOGIC_PRIMARY_MIC:
        io_idx = 0;
        break;

    case AUDIO_ROUTE_LOGIC_SECONDARY_MIC:
        io_idx = 1;
        break;

    case AUDIO_ROUTE_LOGIC_PRIMARY_SPEAKER:
        io_idx = 2;
        break;
...
static bool app_eq_fitting_send_hw_eq(T_AUDIO_CATEGORY category)
{
    ...
    for (i = 0; i < pysical_path_group.physical_path_num; i++)
    {
        ...
        uint8_t io_idx = app_eq_fitting_get_io_idx(p_path->logic_io_type);
        ...
        if (bit_mask & BIT3) // SECONDARY_VOICE_MIC
        {
            io_idx = 1;
        }
        ...
        if (physical_io_type == AUDIO_ROUTE_PHYSICAL_IO_MIC)
        {
            ...
            channel = p_path->attr.mic_attr.adc_ch;
        }
...
static void app_eq_fitting_set_test_mode_tmp_eq_advanced(void)
{
    if (test_mode_tmp_eq_advanced)
    {
        ...
        for (i = 0; i < eq_num; i++)
        {
            ...
            else if (eq_para->device_type == HW_EQ_DEVICE_SECONDARY_MIC)
            {
                eq_type = CODEC_EQ_CONFIG_PATH_ADC;
                eq_channel = 1;
            }
    ...
    case CMD_HW_EQ_TEST_MODE:
        {
            ...
            else if (test_mode_status == HW_EQ_EXIT_TEST_MODE)
            {
                ...
                if (app_eq_fitting_is_streaming(AUDIO_STREAM_TYPE_PLAYBACK))
                {
                    audio_track_restart(app_db.br_link[app_a2dp_get_active_idx()].a2dp.track_handle);
                }

                if (app_eq_fitting_is_streaming(AUDIO_STREAM_TYPE_VOICE))
                {
                    audio_track_restart(app_db.br_link[app_hfp_get_active_idx()].sco.track_handle);
                }
            ...
            if (audio_track_stream_type_get(param->track_state_changed.handle, &stream_type))
            {
                if ((state == AUDIO_TRACK_STATE_STARTED) || (state == AUDIO_TRACK_STATE_RESTARTED))
                {
                    ...
                    else
                    {
                        ...
                        if (src_route != SOURCE_ROUTE_SPDIF)
#endif
                        {
                            if (stream_type == AUDIO_STREAM_TYPE_PLAYBACK)
                            {
