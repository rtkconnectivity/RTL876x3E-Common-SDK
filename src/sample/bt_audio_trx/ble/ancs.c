...
static uint16_t call_conn_handle = 0;
static uint32_t call_notification_uid = 0;
...
void app_handle_notification_attribute_data(T_APP_ANCS_LINK *p_ancs_link)
{
    if (p_ancs_link->notification_attr.attribute_id != DS_NOTIFICATION_ATTR_ID_NEGATIVE_ACTION_LABEL)
    {
        ...
        if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_APP_IDENTIFIER)
        {
            ...
            {
                p_ancs_link->app_type = ANCS_NOTIFICATION_TYPE_TENCENT_WECHAT;
            }
            else if (0 == memcmp(p_value, "com.apple.MobileSMS", 20))
            {
                p_ancs_link->app_type = ANCS_NOTIFICATION_TYPE_MOBILE_SMS;
            }
            else if (0 == memcmp(p_value, "com.apple.mobilephone", 22))
            {
                p_ancs_link->app_type = ANCS_NOTIFICATION_TYPE_MOBILE_PHONE;
            }
            else if (0 == memcmp(p_value, "com.tencent.mqq", 16))
            {
                p_ancs_link->app_type = ANCS_NOTIFICATION_TYPE_TENCENT_QQ;
            }
        ...
        else if (p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_TITLE ||
                 p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_SUB_TITLE ||
                 p_ancs_link->notification_attr.attribute_id == DS_NOTIFICATION_ATTR_ID_MESSAGE)
        {
            if (p_ancs_link->app_type == ANCS_NOTIFICATION_TYPE_TENCENT_WECHAT)
            {
                APP_PRINT_INFO1("Wechat: message %s", TRACE_STRING(p_value));
            }
            else if (p_ancs_link->app_type == ANCS_NOTIFICATION_TYPE_MOBILE_SMS)
            {
                APP_PRINT_INFO1("MobileSMS: message %s", TRACE_STRING(p_value));
            }
            else if (p_ancs_link->app_type == ANCS_NOTIFICATION_TYPE_MOBILE_PHONE)
            {
                APP_PRINT_INFO1("MobileCall: Name or Phone number %s", TRACE_STRING(p_value));
            }
            else if (p_ancs_link->app_type == ANCS_NOTIFICATION_TYPE_TENCENT_QQ)
            {
...
{
    APP_PRINT_INFO3("ANCS_DATA_SOURCE: conn_handle 0x%x, len %d, data %b", conn_handle, len,
                                        ...
                                        T_ANCS_CLIENT_NOTIFICATION_SOURCE *p_notify_src)
{
    ...
    {
#endif
        if (p_notify_src->category_id == NS_CATEGORY_ID_INCOMING_CALL)
        {
            call_conn_handle = conn_handle;
            call_notification_uid = p_notify_src->notification_uid;
...
{
    bool ret = false;
    uint16_t conn_handle = call_conn_handle;
    uint32_t noti_uid = call_notification_uid;
    uint8_t action_id = CP_ACTION_ID_POSITIVE;

    ret = ancs_client_cp_perform_notification_action(conn_handle, noti_uid, action_id);
...
bool ancs_reject_incoming_call(void)
{
    ...
    uint16_t conn_handle = call_conn_handle;
    uint32_t noti_uid = call_notification_uid;
    uint8_t action_id = CP_ACTION_ID_NEGATIVE;
    ...
    case GATT_MSG_ANCS_CLIENT_WRITE_CP_RESULT:
        {
            ...
            APP_PRINT_INFO1("GATT_MSG_ANCS_CLIENT_WRITE_CP_RESULT: cause 0x%x", p_write_result->cause);
        }
        break;
    ...
    case GATT_MSG_ANCS_CLIENT_CCCD_CFG_RESULT:
        {
            ...
            {
                APP_PRINT_INFO1("GATT_MSG_ANCS_CLIENT_CCCD_CFG_RESULT: CCCD enable state %d", p_cfg_result->enable);
            }
        }
        break;
