                         ...
                         uint16_t ccc_bits)
{
    ...
    case GATT_SRV_HID_MM_KB_INPUT_CCCD_INDEX:
        {
            callback_data.msg_data.not_ind_data.index = GATT_SRV_HID_MM_KB_INPUT_CCCD_INDEX;
            if (ccc_bits & GATT_PDU_TYPE_NOTIFICATION)
            {
                callback_data.msg_data.not_ind_data.value = NOTIFY_ENABLE;
            }
 ...
 * \endcode
 */
bool hids_send_report(uint8_t conn_handle, uint8_t report_id, uint8_t *p_data,
                      uint16_t data_len)
{
    bool ret = true;
    PROFILE_PRINT_INFO3("hids_send_report: conn_handle %d, report id %d, data_len %d", conn_handle,
                        report_id, data_len);
    if (report_id == HOGP_MM_REPORT_ID)
    {
        return gatt_svc_send_data(conn_handle, L2C_FIXED_CID_ATT, srv_id_local,
                                  GATT_SRV_HID_MM_KB_INPUT_INDEX, p_data,
