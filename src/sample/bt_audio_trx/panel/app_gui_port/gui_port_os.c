...
static uint8_t gui_port_os_timer_id;
static uint8_t timer_idx_fb_skip = 0;
static uint32_t fb_skip_count;
#define GUI_FB_SKIP_FRAME_NUM   1
#define GUI_FB_SKIP_FRAME_DURATION   3*1000
...
void app_gui_set_bond_flag(void)
{
    ...
    {
        app_start_timer(&timer_idx_fb_skip, "fb skip",
                        gui_port_os_timer_id, GUI_PORT_SKIP_FB, 0, false,
                        GUI_FB_SKIP_FRAME_DURATION);
...
bool port_mq_send(void *handle, void *buffer, uint32_t size, uint32_t timeout)
{
    ...
    {
        APP_PRINT_ERROR2("gui_mq_send fail io queue line = %d,0x%x", __LINE__, handle);
...
{
#if 0 //renee removed for not support in RTL8763E
...
void gui_port_os_add_exe_to_gui_task()
{
    ...
    // fix bond issue   BB2PLUSBUG-376
    if (ble_bond_new_device_flag)
    {
        fb_skip_count++;
        if (fb_skip_count % (GUI_FB_SKIP_FRAME_NUM + 1))
        {
            os_delay(20);
        }
    }
...
static void gui_port_os_timer_cback(uint8_t timer_evt, uint16_t param)
{
    gui_log("gui_port_os_timer_cback: timer_evt %d, param 0x%x", timer_evt, param);
    switch (timer_evt)
    {
    case GUI_PORT_SKIP_FB:
        {
...
void gui_port_os_init(void)
{
    ...
    app_timer_reg_cb(gui_port_os_timer_cback, &gui_port_os_timer_id);
}
