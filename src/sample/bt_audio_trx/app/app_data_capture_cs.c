...
uint16_t dsp_capture_data_state = 0;
...
typedef struct
{
    ...
    T_AUDIO_ROUTE_LOGIC_IO_TYPE mic;
...
#if F_APP_SAIYAN_MODE | F_APP_SUPPORT_CAPTURE_ACOUSTICS_MP
...
void app_data_capture_start_process(T_CAPTURE_HEADER *param, uint8_t app_idx)
{
    ...
    if (app_db.br_link[app_idx].acl.role == BT_LINK_ROLE_MASTER)
    {
        is_master = true;
        bt_link_role_switch(app_db.br_link[app_idx].bd_addr, false); // master -> slave
    }
...
void app_data_capture_stop_process(uint8_t app_idx)
{
    ...
    if ((app_db.br_link[app_idx].acl.role == BT_LINK_ROLE_SLAVE) && (is_master == true))
    {
        is_master = false;
        bt_link_role_switch(app_db.br_link[app_idx].bd_addr, true); // slave -> master
    }
...
static uint8_t app_data_capture_a2dp_hfp_mask_check(uint8_t app_idx)
{
    ...
    {
        if (app_db.br_link[app_idx].a2dp.track_handle)
        {
            audio_track_state_get(app_db.br_link[app_idx].a2dp.track_handle, &state);
        ...
        if (app_db.br_link[app_idx].sco.track_handle)
        {
            audio_track_state_get(app_db.br_link[app_idx].sco.track_handle, &state);
            if ((state == AUDIO_TRACK_STATE_CREATED) ||
                (state == AUDIO_TRACK_STATE_STARTED) ||
                (state == AUDIO_TRACK_STATE_RESTARTED))
            {
...
static T_APP_AUDIO_CATEGORY_LOGIC_MIC app_data_capture_get_category_mic(uint8_t logic_mic)
{
    ...
    case LOGIC_FF_MIC:
        {
            ...
            category_mic.mic      = AUDIO_ROUTE_LOGIC_PRIMARY_MIC;
    ...
    case LOGIC_FB_MIC:
        {
            category_mic.category = AUDIO_CATEGORY_ANC;
            category_mic.mic      = AUDIO_ROUTE_LOGIC_SECONDARY_MIC;
    ...
    default:
        {
            category_mic.category = AUDIO_CATEGORY_NUMBER;
            category_mic.mic      = AUDIO_ROUTE_LOGIC_IO_NUM;
        }
    }
    return category_mic;
}
                ...
                case APP_DATA_CAPTURE_ACOUSTICS_MP:
                    {
                        ...
                        T_AUDIO_ROUTE_PHYSICAL_MIC record_primary_mic;
                        T_AUDIO_ROUTE_PHYSICAL_MIC record_secondary_mic;

#ifndef TARGET_RTL8773DO
                        audio_probe_dsp_test_bin_set(true);
#endif
                        app_data_capture_register();
                        memset(&physical_mic, 0, sizeof(physical_mic));
                        ...
                        // mic1
                        category_mic = app_data_capture_get_category_mic(user_mic_sbc_record.logic_mic1);
                        audio_category = category_mic.category;
                        logical_mic = category_mic.mic;

                        if ((audio_category == AUDIO_CATEGORY_VOICE) ||
                            (audio_category == AUDIO_CATEGORY_ANC) ||
                            (audio_category == AUDIO_CATEGORY_APT))
                        {
                            app_audio_route_physical_mic_get(audio_category,
                                                             logical_mic,
                                                             &physical_mic);
                        }

                        app_audio_route_physical_mic_get(AUDIO_CATEGORY_RECORD,
                                                         AUDIO_ROUTE_LOGIC_PRIMARY_MIC,
                                                         &record_primary_mic);

                        record_primary_mic.mic_sel = physical_mic.mic_sel;
                        record_primary_mic.mic_type = physical_mic.mic_type;
                        record_primary_mic.adc_ch = physical_mic.adc_ch;

                        app_audio_route_physical_mic_set(AUDIO_CATEGORY_RECORD,
                                                         AUDIO_ROUTE_LOGIC_PRIMARY_MIC,
                                                         record_primary_mic);

                        // mic2
                        if (user_mic_sbc_record.logic_mic2 != LOGIC_INVALID_MIC)
                        {
                            category_mic = app_data_capture_get_category_mic(user_mic_sbc_record.logic_mic2);
                            audio_category = category_mic.category;
                            logical_mic = category_mic.mic;
                            ...
                            app_audio_route_physical_mic_get(AUDIO_CATEGORY_RECORD,
                                                             ...
                                                             &record_secondary_mic);

                            record_secondary_mic.mic_sel = physical_mic.mic_sel;
                            record_secondary_mic.mic_type = physical_mic.mic_type;
                            record_secondary_mic.adc_ch = physical_mic.adc_ch;
