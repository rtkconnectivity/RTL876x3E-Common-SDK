...
static void key_gpio_key0_debounce_handler(void)
{
    ...
    if (app_key_cfg.key_enable_mask & KEY0_MASK)
    {
        ...
        {
            key_status = hal_gpio_get_input_level(app_key_cfg.key_pinmux[0]);

            if (key_status == key0_state)
            {
                /* Enable GPIO interrupt */
                APP_PRINT_ERROR1("key_gpio_key0_debounce_handler: ignore for same level, key0_state 0x%x",
                                 key0_state);
                app_dlps_enable(APP_DLPS_ENTER_CHECK_GPIO);
                return;
            }

            key0_state = key_status;
        }
    }
    else
    {
    ...
    if (app_key_cfg.key_high_active_mask & BIT0)
    {
        if (key_status) //Button pressed
        {
            if (!app_key_cfg.mfb_replace_key0)
            {
                hal_gpio_irq_change_polarity(app_key_cfg.key_pinmux[0], GPIO_IRQ_ACTIVE_LOW);
            }
        ...
        else //Button released
        {
            ...
            if (!app_key_cfg.mfb_replace_key0)
            {
                hal_gpio_irq_change_polarity(app_key_cfg.key_pinmux[0], GPIO_IRQ_ACTIVE_HIGH);
            }
    ...
    if (!app_key_cfg.mfb_replace_key0)
    {
        ...
        {
            /* Enable GPIO interrupt */
            hal_gpio_irq_enable(app_key_cfg.key_pinmux[0]);
        }
    }
}
...
void key_gpio_initial(void)
{
    ...
    if (MAX_KEY_NUM != 0)
    {
        ...
        if (app_key_cfg.key_enable_mask & KEY0_MASK)
        {
            key0_state = ~(app_key_cfg.key_high_active_mask & BIT0);
            pull_mode = (app_key_cfg.key_high_active_mask & BIT(0)) ? GPIO_PULL_DOWN : GPIO_PULL_UP;
            hal_gpio_init_pin(app_key_cfg.key_pinmux[0], GPIO_TYPE_CORE, GPIO_DIR_INPUT, pull_mode);
            ...
            hal_gpio_set_up_irq(app_key_cfg.key_pinmux[0], GPIO_IRQ_LEVEL, key0_irq_ploarity, false);
        }
        ...
        for (i = 1; i < MAX_KEY_NUM; i++)
        {
            if ((app_key_cfg.key_enable_mask & BIT(i))
                && (app_key_cfg.key_high_active_mask & BIT(i)))
            {
                hal_gpio_init_pin(app_key_cfg.key_pinmux[i], GPIO_TYPE_CORE, GPIO_DIR_INPUT, GPIO_PULL_DOWN);
                hal_gpio_set_up_irq(app_key_cfg.key_pinmux[i], GPIO_IRQ_EDGE, GPIO_IRQ_ACTIVE_HIGH, true);
            }
        }
        ...
        {
            if ((app_key_cfg.key_enable_mask & BIT(i))
                && ((app_key_cfg.key_high_active_mask & BIT(i)) == 0))
            {
                hal_gpio_init_pin(app_key_cfg.key_pinmux[i], GPIO_TYPE_CORE, GPIO_DIR_INPUT, GPIO_PULL_UP);
                hal_gpio_set_up_irq(app_key_cfg.key_pinmux[i], GPIO_IRQ_EDGE, GPIO_IRQ_ACTIVE_LOW, true);
            }
        }
        ...
        for (i = 0; i < MAX_KEY_NUM; i++)
        {
            ...
            if (app_key_cfg.key_enable_mask & BIT(i))
            {
                if (app_key_cfg.key_high_active_mask & BIT(i))
                {
                    Pad_WakeupPolarityValue(app_key_cfg.key_pinmux[i], PAD_WAKEUP_POL_HIGH);
                }
                else
                {
                    Pad_WakeupPolarityValue(app_key_cfg.key_pinmux[i], PAD_WAKEUP_POL_LOW);
                }

                if (i == 0)
                {
                    hal_gpio_register_isr_callback(app_key_cfg.key_pinmux[0], key_gpio_key0_intr_cb, i);
                }
                else
                {
                    hal_gpio_register_isr_callback(app_key_cfg.key_pinmux[i], key_gpio_intr_cb, i);
                }
                hal_gpio_irq_enable(app_key_cfg.key_pinmux[i]);
            }
        }
    }
}
...
void key_gpio_intr_cb(uint32_t key_index)
{
    ...
    uint8_t gpio_index = app_key_cfg.key_pinmux[key_index];
    uint8_t key_status;
    uint8_t key_status_update_fg = 0;
    T_IO_MSG button_msg;
    ...
    if (key_mask & app_key_cfg.key_high_active_mask) // high active
    {
        if (key_status) //Button pressed
        {
...
void key_gpio_key0_intr_handler(void)
{
    ...
    p_gpio_data->key_status[0] = hal_gpio_get_input_level(app_key_cfg.key_pinmux[0]);
    app_dlps_pad_wake_up_polarity_invert(app_key_cfg.key_pinmux[0]);
...
bool key_gpio_check_power_button_pressed(void)
{
    ...
    {
        return !((bool)(app_key_cfg.key_high_active_mask & BIT0) ^ ((bool)hal_gpio_get_input_level(
                                                                        app_key_cfg.key_pinmux[0])));
    }
}
