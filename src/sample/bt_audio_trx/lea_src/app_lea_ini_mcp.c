...
bool app_lea_ini_mcp_handle_operation(T_MCP_SERVER_WRITE_MEDIA_CP_IND *p_mcp_req)
{
    ...
    APP_PRINT_INFO2("MCP CP: opcode 0x%x, curr media state %d\r\n", opcode,
    ...
    case LE_AUDIO_MSG_MCP_SERVER_READ_IND:
        {
            ...
            if (p_read_ind)
            {
                ...
                case MCS_UUID_CHAR_MEDIA_PLAYER_NAME:
                    {
                        read_cfm.param.media_player_name.p_media_player_name = (uint8_t *)"RTK player";
                ...
                case MCS_UUID_CHAR_TRACK_TITLE:
                    {
                        read_cfm.param.track_title.p_track_title = (uint8_t *)"RTK media trace title";
