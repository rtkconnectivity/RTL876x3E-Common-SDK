...
uint8_t app_lea_bsrc_gen_pbp_adv_data(uint8_t **p_audio_adv_data)
{
    ...
    memcpy(&audio_adv_data[idx], app_cfg_nv.device_name_le, name_len);
    idx += name_len;
...
bool app_lea_bsrc_config(void)
{
    ...
    app_tri_dongle_pa_setting(&periodic_adv_interval_min, &periodic_adv_interval_max);
#else
        ...
        app_db.bsrc_db.state == BROADCAST_SOURCE_STATE_IDLE)
    {
        ...
        if (broadcast_source_set_eadv_param(app_db.bsrc_db.source_handle, 0,
...
bool app_lea_bsrc_establish(void)
{
    ...
    big_param.max_sdu = app_db.bsrc_db.max_sdu;
    if (big_param.num_bis == 1)
    {
        big_param.max_sdu = app_db.bsrc_db.max_sdu * 2;
    }
...
static void app_lea_gen_basic_data(void)
{
...
#if 0
    uint8_t ccid = APP_CONTENT_CONTROL_ID_CHAR_VALUE_GMCS;
#endif
    ...
    TEST_PRINT_INFO0("app_lea_gen_basic_data: success");
#endif
    return;
error:
...
void app_lea_bsrc_sm_cb(T_BROADCAST_SOURCE_HANDLE handle, uint8_t cb_type, void *p_cb_data)
{
    ...
    case MSG_BROADCAST_SOURCE_SETUP_DATA_PATH:
        {
#if F_APP_AUTO_POWER_TEST_LOG
            TEST_PRINT_INFO2("MSG_BROADCAST_SOURCE_SETUP_DATA_PATH: bis_idx %d, cause 0x%x",
    ...
    case MSG_BROADCAST_SOURCE_REMOVE_DATA_PATH:
        {
#if F_APP_AUTO_POWER_TEST_LOG
            TEST_PRINT_INFO2("MSG_BROADCAST_SOURCE_REMOVE_DATA_PATH: bis_conn_handle %d, cause 0x%x",
            ...
            data.path_direction = DATA_PATH_INPUT_FLAG;
            app_lea_handle_bis_data_path_remove(&data);
        }
        break;
    default:
        break;
...
#if CONFIG_REALTEK_APP_BT_AUDIO_TRI_DONGLE
    app_tri_dongle_lea_bsrc_sm_cb(handle, cb_type, p_sm_data);
#endif
}
...
void app_lea_bsrc_init(T_CODEC_CFG_ITEM index, T_QOS_CFG_TYPE qos_type, uint8_t bis_num,
                       T_GAP_LOCAL_ADDR_TYPE local_addr_type, bool encryption, uint16_t presentation_delay)
{
    T_BROADCAST_SOURCE_INFO src_info;

    if (app_db.bsrc_db.source_handle != NULL)
    {
        APP_PRINT_INFO1("app_lea_bsrc_init: use source handle 0x%x", app_db.bsrc_db.source_handle);
    }
    else
    {
    ...
    app_db.bsrc_db.codec_cfg.audio_channel_allocation = AUDIO_LOCATION_FL | AUDIO_LOCATION_FR;
    ...
    {
        app_db.bsrc_db.prefer_qos.retransmission_number = 2;
        app_db.bsrc_db.prefer_qos.presentation_delay = presentation_delay;
        app_db.bsrc_db.prefer_qos.max_transport_latency = 10;
    }

#if F_APP_BT_AUDIO_TRI_DONGLE_LEAUDIO
    app_tri_dongle_qos_setting(index);
    APP_PRINT_INFO2("app_lea_bsrc_init: use pd 0x%x, max_sdu 0x%x",
                    presentation_delay,
                    app_db.bsrc_db.max_sdu);
#endif
