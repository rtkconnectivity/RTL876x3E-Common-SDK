...
const uint32_t prof_relation[6][2] =
{
    {A2DP_SINK_PROFILE_MASK, AVRCP_CONTROLLER_PROFILE_MASK},
    {A2DP_SRC_PROFILE_MASK, AVRCP_TARGET_PROFILE_MASK},
    {HFP_HF_PROFILE_MASK, PBAP_PSE_PROFILE_MASK},
    {HFP_AG_PROFILE_MASK, PBAP_PCE_PROFILE_MASK},
    {HSP_HF_PROFILE_MASK, PBAP_PSE_PROFILE_MASK},
    {HSP_AG_PROFILE_MASK, PBAP_PCE_PROFILE_MASK},
};
...
bool linkback_profile_search_start(uint8_t *bd_addr, uint32_t prof, bool is_special)
{
    ...
    case AVRCP_TARGET_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_AV_REMOTE_CONTROL_TARGET;
        }
        break;

    case AVRCP_CONTROLLER_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_AV_REMOTE_CONTROL_CONTROLLER;
        }
        break;

    case HFP_AG_PROFILE_MASK:
        {
    ...
    case HSP_AG_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_HANDSFREE_AUDIO_GATEWAY;
        }
        break;

    case HSP_HF_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_HANDSFREE;
        }
        break;
    ...
    case PBAP_PSE_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_PBAP_PSE;
        }
        break;

    case PBAP_PCE_PROFILE_MASK:
        {
            uuid.uuid_16 = UUID_PBAP_PCE;
        }
        break;
...
#if F_APP_MULTI_2EP_1_BR_1_BLE_HID
    case HID_PROFILE_MASK:
        ret = bt_hid_host_connect_req(bd_addr, BT_HID_HOST_REPORT_PROTO_MODE);
        break;
#endif
...
void linkback_profile_disconnect_start(uint8_t *bd_addr, uint32_t profs)
{
    ...
    if (0 != profs)
    {
        if (profs & (A2DP_SINK_PROFILE_MASK | A2DP_SRC_PROFILE_MASK))
        {
            bt_a2dp_disconnect_req(bd_addr);
        }

        if (profs & (AVRCP_PROFILE_MASK | AVRCP_PROFILE_MASK))
        {
            bt_avrcp_disconnect_req(bd_addr);
        }

        if (profs & (HFP_HF_PROFILE_MASK | HSP_HF_PROFILE_MASK))
        {
            bt_hfp_ag_disconnect_req(bd_addr);
        }

        if (profs & (HFP_AG_PROFILE_MASK | HSP_AG_PROFILE_MASK))
        {
        ...
        if (profs & HID_PROFILE_MASK)
        {
            bt_hid_host_disconnect_req(bd_addr);
        }
#endif
...
void linkback_todo_queue_insert_force_node(uint8_t *bd_addr, uint32_t plan_profs,
                                           bool is_special, bool check_bond_flag,
                                           uint32_t retry_timeout, bool is_group_member)
{
    T_LINKBACK_NODE_ITEM *p_item;
...
void linkback_load_bond_list(uint8_t skip_node_num, uint16_t retry_timeout)
{
    ...
    max_load_num = app_bt_policy_cfg.maximum_linkback_number;
    ...
    if (linkback_active_node.linkback_node.retry_timeout > time_used_ms)
    {
        ...
        else
        {
            linkback_todo_queue_insert_force_node(linkback_active_node.linkback_node.bd_addr,
                                                  linkback_active_node.linkback_node.plan_profs,
                                                  linkback_active_node.linkback_node.is_special,
