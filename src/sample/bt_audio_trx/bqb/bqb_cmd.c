...
bool bqb_avdtp(const char *cmd_str, char *buf, size_t buf_len)
{
    ...
    else if (!strcmp(subcmd, "connect"))
    {
        ...
        if (!strcmp(subcmd, "signal_snk"))
        {
            action = BQB_ACTION_AVDTP_CONNECT_SIGNAL_SNK;
        }
        else if (!strcmp(subcmd, "signal_src"))
        {
            action = BQB_ACTION_AVDTP_CONNECT_SIGNAL_SRC;
        }
...
bool bqb_rfcomm(const char *cmd_str, char *buf, size_t buf_len)
{
    ...
    if (!strcmp(subcmd, "connect"))
    {
        ...
        p_param = subcmd + param_len + 1;
        server_chann = (uint8_t)strtol(cli_param_get(p_param, 0, &param_len), NULL, 0);
        p_param += param_len + 1;
        ...
        if (param_buf != NULL)
        {
            ...
            LE_UINT8_TO_STREAM(p, action);
            LE_UINT8_TO_STREAM(p, server_chann);
            ARRAY_TO_STREAM(p, addr, 6);
...
bool bqb_pbap(const char *cmd_str, char *buf, size_t buf_len)
{
    ...
    if (!strcmp(subcmd, "connect_rfc"))
    {
        ...
        server_chann = (uint8_t)strtol(cli_param_get(p_param, 0, &param_len), NULL, 0);
        p_param += param_len + 1;
        feature_flag = (uint8_t)strtol(cli_param_get(p_param, 0, &param_len), NULL, 0);
        p_param += param_len + 1;
        ...
        action = BQB_ACTION_PBAP_CONN_RFC;
    }
    else if (!strcmp(subcmd, "connect_l2c"))
    {
        if (param_num != 9)
        {
            goto err;
        ...
        p_param = subcmd + param_len + 1;
        l2c_psm = (uint16_t)strtol(cli_param_get(p_param, 0, &param_len), NULL, 0);
        p_param += param_len + 1;
        feature_flag = (uint8_t)strtol(cli_param_get(p_param, 0, &param_len), NULL, 0);
        p_param += param_len + 1;
        ...
        action = BQB_ACTION_PBAP_CONN_L2C;
    }
    else if (!strcmp(subcmd, "vcard"))
    {
        if (param_num != 8)
        {
            goto err;
    ...
    else if (!strcmp(subcmd, "set_phone_book"))
    {
        ...
        action = BQB_ACTION_PBAP_SET_PHONE_BOOK;
    }
    else if (!strcmp(subcmd, "pull_phone_book"))
    {
        if (param_num != 7)
        {
            goto err;
        ...
        action = BQB_ACTION_PBAP_PULL_PHONE_BOOK;
    }
    else
    {
        goto err;
    ...
    if (param_buf != NULL)
    {
        ...
        if (action == BQB_ACTION_PBAP_CONN_RFC)
        {
            LE_UINT8_TO_STREAM(p, server_chann);
            LE_UINT8_TO_STREAM(p, feature_flag);
        }
        else if (action == BQB_ACTION_PBAP_CONN_L2C)
        {
            LE_UINT16_TO_STREAM(p, l2c_psm);
            LE_UINT8_TO_STREAM(p, feature_flag);
        }
...
bool bqb_map(const char *cmd_str, char *buf, size_t buf_len)
{
    ...
    if (!strcmp(subcmd, "connect_rfc"))
    {
        ...
        action = BQB_ACTION_MAP_CONNECT_MAS_RFC;
    }
    else if (!strcmp(subcmd, "connect_l2c"))
    {
        if (param_num != 8)
        ...
        action = BQB_ACTION_MAP_CONNECT_MAS_L2C;
    }
    else if (!strcmp(subcmd, "disconnect"))
    {
        if (param_num != 7)
    ...
    if (param_buf != NULL)
    {
        ...
        else if (action == BQB_ACTION_MAP_CONNECT_MAS_RFC)
        {
            LE_UINT8_TO_STREAM(p, server_chann);
        }
        else if (action == BQB_ACTION_MAP_CONNECT_MAS_L2C)
        {
            LE_UINT16_TO_STREAM(p, l2c_psm);
        }
        ARRAY_TO_STREAM(p, addr, 6);
