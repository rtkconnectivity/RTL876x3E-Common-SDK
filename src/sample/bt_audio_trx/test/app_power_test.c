...
static void app_power_test_bt_cback(T_BT_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    case BT_EVENT_REMOTE_CONN_CMPL:
        {
            TEST_PRINT_INFO2("set_bd_addr: role %d, local_addr %s",
                             app_cfg_nv.bud_role,
                             TRACE_BDADDR(app_cfg_nv.bud_local_addr));
            TEST_PRINT_INFO2("connected: bud_role %d, b2s %d",
                             app_cfg_nv.bud_role,
    ...
    case BT_EVENT_AVRCP_CONN_CMPL:
        {
            ...
            if (plink)
            {
                TEST_PRINT_INFO2("AVRCP connected: bud_role %d, b2s %d",
                                 app_cfg_nv.bud_role,
                                 app_link_get_b2s_link_num());
                TEST_PRINT_INFO2("AVRCP connected: b2s, bd_addr %s, profs 0x%08x",
    ...
    case BT_EVENT_HFP_CONN_CMPL:
        {
            ...
            if (plink)
            {
                TEST_PRINT_INFO2("HF connected: bud_role %d, b2s %d",
                                 app_cfg_nv.bud_role,
                                 app_link_get_b2s_link_num());
                TEST_PRINT_INFO2("HF connected: b2s, bd_addr %s, profs 0x%08x",
                                 TRACE_BDADDR(plink->bd_addr),
                                 plink->connected_profile);
            }
        }
    ...
    case BT_EVENT_HFP_AG_CONN_CMPL:
        {
            T_APP_BR_LINK *plink = app_link_find_b2s_link(param->hfp_ag_conn_cmpl.bd_addr);
            if (plink)
            {
                TEST_PRINT_INFO2("AG connected: bud_role %d, b2s %d",
                                 app_cfg_nv.bud_role,
                                 app_link_get_b2s_link_num());
                TEST_PRINT_INFO2("AG connected: b2s, bd_addr %s, profs 0x%08x",
                                 TRACE_BDADDR(plink->bd_addr),
                                 plink->connected_profile);
            }
        }
    ...
    case BT_EVENT_A2DP_CONN_CMPL:
        {
            ...
            if (plink)
            {
                TEST_PRINT_INFO2("A2DP connected: bud_role %d, b2s %d",
                                 app_cfg_nv.bud_role,
                                 app_link_get_b2s_link_num());
                TEST_PRINT_INFO2("A2DP connected: b2s, bd_addr %s, profs 0x%08x",
                                 TRACE_BDADDR(plink->bd_addr),
                                 plink->connected_profile);
            }
        }
    ...
    case BT_EVENT_AVRCP_PLAY_STATUS_CHANGED:
        {
            TEST_PRINT_INFO1("app_power_test_bt_cback: AVRCP play status %d",
    ...
    case BT_EVENT_HFP_CALL_STATUS:
        {
            TEST_PRINT_INFO1("app_power_test_bt_cback: HF call status 0x%02x",
    ...
    case BT_EVENT_HFP_AG_CALL_STATUS_CHANGED:
        {
            TEST_PRINT_INFO1("app_power_test_bt_cback: AG call status 0x%02x",
                             param->hfp_ag_call_status_changed.curr_status);
        }
        break;
