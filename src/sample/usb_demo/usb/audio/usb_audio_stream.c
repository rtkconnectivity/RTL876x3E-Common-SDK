...
#if UAS_DBG_LOG_EN
    USB_PRINT_INFO2("usb_audio_stream_get_by_id: dir %d, pipe %d", stream_id.u.dir, stream_id.u.pipe);
#endif
...
static bool usb_audio_stream_create(T_USB_AUDIO_STREAM *stream, T_STREAM_ATTR attr)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_create: stream is NULL");
        return NULL;
    }

    if (usb_audio_stream_state_get(stream) != STREAM_STATE_IDLE)
    {
        APP_PRINT_ERROR1("usb_audio_stream_create: state 0x%x", stream->state);
        return NULL;
    }
...
static bool usb_audio_stream_release(T_USB_AUDIO_STREAM *stream)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_release: stream is NULL");
        return NULL;
    }
    ...
    {
        APP_PRINT_ERROR1("usb_audio_stream_release: state 0x%x", stream->state);
        return NULL;
    }
...
bool usb_audio_stream_start(uint32_t id, bool sync)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_start: stream is NULL");
        return false;
    }
    ...
    if (ual == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_start: ual is NULL");
        return false;
    }
    ...
    {
        APP_PRINT_ERROR1("usb_audio_stream_start: state 0x%x", state);
        if (state == STREAM_STATE_IDLE)
        {
...
void usb_audio_stream_start_complete(uint32_t id)
{
    ...
    if (usb_audio_stream_state_get(stream) == STREAM_STATE_INITED)
    {
        ...
        usb_audio_stream_state_set(stream, STREAM_STATE_ACTIVE);
        stream->active_ual = ual;
        usb_audio_stream_ctrl(stream, CTRL_CMD_VOL_CHG, &stream->ctrl);
    }
}
...
bool usb_audio_stream_stop(uint32_t id, bool sync)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_stop: stream is NULL");
        return false;
    }
    ...
    if (state < STREAM_STATE_ACTIVE)
    {
        ...
        APP_PRINT_ERROR1("usb_audio_stream_stop: state 0x%x", state);
        return false;
    }
...
static bool usb_audio_stream_xmit(T_USB_AUDIO_STREAM *stream)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_xmit: stream is NULL");
        return false;
    }
    ...
    if (usb_audio_stream_state_get(stream) < STREAM_STATE_ACTIVE)
    {
        APP_PRINT_ERROR1("usb_audio_stream_xmit: state 0x%x", state);
        return false;
    }
...
#if UAS_DBG_LOG_EN
    APP_PRINT_INFO4("usb_audio_stream_pipe_xmit_in: buf 0x%x, len 0x%x, stream 0x%x, remain_len 0x%x",
                    buf, len, stream, data_len);
#endif
...
static bool usb_audio_stream_ctrl(T_USB_AUDIO_STREAM *stream, uint8_t cmd, void *param)
{
    ...
    if (stream == NULL)
    {
        APP_PRINT_ERROR0("usb_audio_stream_ctrl: stream is NULL");
        return false;
    }
...
#if UAS_DBG_LOG_EN
    APP_PRINT_INFO5("usb_audio_stream_pipe_xmit_out: stream 0x%x, pool 0x%x, label %d, buf 0x%x, len 0x%x",
                    stream, &(stream->pool), label, buf, len);
#endif
    ...
    case USB_AUDIO_STREAM_EVT_VOL_GET:
        {
            ...
            APP_PRINT_INFO2("usb_audio_stream_evt_handle: EVT_VOL_GET, cur 0x%x, range 0x%x",
                            stream->ctrl.vol.cur, stream->ctrl.vol.range);
        }
        break;
    ...
    case USB_AUDIO_STREAM_EVT_VOL_SET:
        {
            ...
            APP_PRINT_INFO2("usb_audio_stream_evt_handle: EVT_VOL_SET, cur 0x%x, range 0x%x",
                            stream->ctrl.vol.cur, stream->ctrl.vol.range);
