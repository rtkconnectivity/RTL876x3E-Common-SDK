...
/** @defgroup  PBAP_DEMO_MAIN PBAP Demo Main
    * @brief Main file to initialize hardware and BT stack and start task scheduling
    * @{
    */

#define PBAP_DEMO_DEFAULT_PAGESCAN_WINDOW             0x12
#define PBAP_DEMO_DEFAULT_PAGESCAN_INTERVAL           0x800 //0x800
#define PBAP_DEMO_DEFAULT_PAGE_TIMEOUT                0x4000
#define PBAP_DEMO_DEFAULT_SUPVISIONTIMEOUT            0x1f40 //0x7D00
#define PBAP_DEMO_DEFAULT_INQUIRYSCAN_WINDOW          0x12
#define PBAP_DEMO_DEFAULT_INQUIRYSCAN_INTERVAL        0x800 //0x1000
...
static void app_bt_gap_init(void)
{
    ...
    uint16_t supervision_timeout = PBAP_DEMO_DEFAULT_SUPVISIONTIMEOUT;
    ...
    uint16_t pagescan_interval = PBAP_DEMO_DEFAULT_PAGESCAN_INTERVAL;
    uint16_t pagescan_window = PBAP_DEMO_DEFAULT_PAGESCAN_WINDOW;
    uint16_t page_timeout = PBAP_DEMO_DEFAULT_PAGE_TIMEOUT;

    uint8_t inquiryscan_type = GAP_INQUIRY_SCAN_TYPE_STANDARD;
    uint16_t inquiryscan_window = PBAP_DEMO_DEFAULT_INQUIRYSCAN_WINDOW;
    uint16_t inquiryscan_interval = PBAP_DEMO_DEFAULT_INQUIRYSCAN_INTERVAL;
...
{
    /* System Manager */
    sys_mgr_init(pbap_demo_evt_queue_handle);
...
{
    Pinmux_Config(PBAP_DEMO_UART_TX, UART0_TX);
    Pad_Config(PBAP_DEMO_UART_TX,
               PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);

    Pinmux_Config(PBAP_DEMO_UART_RX, UART0_RX);
    Pad_Config(PBAP_DEMO_UART_RX,
               PAD_PINMUX_MODE, PAD_IS_PWRON, PAD_PULL_UP, PAD_OUT_DISABLE, PAD_OUT_LOW);
}
...
static void driver_init(void)
{
    ...
    console_uart_config.uart_rx_pinmux = PBAP_DEMO_UART_RX;
    console_uart_config.uart_tx_pinmux = PBAP_DEMO_UART_TX;
    console_uart_config.rx_wake_up_pinmux = PBAP_DEMO_UART_RX;
    ...
    console_param.tx_wakeup_pin = PBAP_DEMO_UART_TX;
    console_param.rx_wakeup_pin = PBAP_DEMO_UART_RX;
