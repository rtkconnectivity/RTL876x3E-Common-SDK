...
/** @defgroup  BT_AUDIO_MAIN BT Audio Main
...
static void app_bt_gap_init(void)
{
    ...
    uint16_t supervision_timeout = BT_AUDIO_DEFAULT_SUPVISIONTIMEOUT;
    ...
    uint16_t pagescan_interval = BT_AUDIO_DEFAULT_PAGESCAN_INTERVAL;
    uint16_t pagescan_window = BT_AUDIO_DEFAULT_PAGESCAN_WINDOW;
    uint16_t page_timeout = BT_AUDIO_DEFAULT_PAGE_TIMEOUT;

    uint8_t inquiryscan_type = GAP_INQUIRY_SCAN_TYPE_STANDARD;
    uint16_t inquiryscan_window = BT_AUDIO_DEFAULT_INQUIRYSCAN_WINDOW;
    uint16_t inquiryscan_interval = BT_AUDIO_DEFAULT_INQUIRYSCAN_INTERVAL;
    ...
    gap_set_bd_addr(bd_addr_dut1);
    gap_br_set_param(GAP_BR_PARAM_NAME, GAP_DEVICE_NAME_LEN, "bt audio src");
    memcpy(app_db.remote_addr, bd_addr_dut2, 6);
#else
    gap_set_bd_addr(bd_addr_dut2);
    gap_br_set_param(GAP_BR_PARAM_NAME, GAP_DEVICE_NAME_LEN, "bt audio snk");
    memcpy(app_db.remote_addr, bd_addr_dut1, 6);
#endif
}
...
#if (BT_AUDIO_DEMO_ROLE == 1)
    app_bt_audio_a2dp_init(BT_A2DP_ROLE_SRC);
#else
    app_bt_audio_a2dp_init(BT_A2DP_ROLE_SNK);
#endif
    app_bt_audio_avrcp_init();
#endif

#if (HFP_DEMO_SUPPORT == 1)
#if (BT_AUDIO_DEMO_ROLE == 1)
    app_bt_audio_hfp_init(BT_AUDIO_HFP_ROLE_AG);
#else
    app_bt_audio_hfp_init(BT_AUDIO_HFP_ROLE_HF);
#endif
#endif
}
...
{
    /* System Manager */
    sys_mgr_init(bt_audio_evt_queue_handle);
 ...
 * @return       void
 */
void bt_audio_main_task(void *p_param)
{
    uint8_t event;

    gap_start_bt_stack(bt_audio_evt_queue_handle, bt_audio_io_queue_handle,
                       BT_AUDIO_MAX_NUMBER_OF_GAP_MESSAGE);

    while (true)
    {
        if (os_msg_recv(bt_audio_evt_queue_handle, &event, 0xFFFFFFFF) == true)
        {
            if (EVENT_GROUP(event) == EVENT_GROUP_IO)
            {
                T_IO_MSG io_msg;

                if (os_msg_recv(bt_audio_io_queue_handle, &io_msg, 0) == true)
                {
 ...
 * @note     There is only one task in BT Audio APP, thus only one APP task is init here
 * @return   void
 */
void task_init(void)
{
    ...
    os_msg_queue_create(&bt_audio_io_queue_handle, "ioQ", BT_AUDIO_MAX_NUMBER_OF_IO_MESSAGE,
                        sizeof(T_IO_MSG));
    os_msg_queue_create(&bt_audio_evt_queue_handle, "evtQ", BT_AUDIO_MAX_NUMBER_OF_EVENT_MESSAGE,
                        sizeof(unsigned char));

    os_task_create(&app_task_handle, "bt_audio_main_task", bt_audio_main_task, NULL, 1024 * 3, 1);
}
