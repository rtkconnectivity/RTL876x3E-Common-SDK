 ...
 *                         Header files
 *============================================================================*/
#if (F_GFPS_COMMON_LE_DEVICE_SUPPORT && F_GFPS_COMMON_LE_L2CAP_CHANNEL)
#include "gfps.h"
#include "trace.h"
#include "string.h"
#include "app_main.h"
...
//            gfps_msg_conn_handle = p_info->conn_handle;
            ...
            {
                APP_PRINT_TRACE2("app_gfps_psm_msg_ecfc_callback: GAP_ECFC_CONN_CMPL %d, cid 0x%x",
                                 i, p_info->cid[i]);
            }
        }
        break;
...
//            gfps_eatt_conn_handle = p_info->conn_handle;
            ...
            for (uint8_t i = 0; i < p_info->cid_num; i++)
            {
                APP_PRINT_TRACE2("app_gfps_psm_eatt_callback: GAP_ECFC_CONN_CMPL %d, cid 0x%x",
                                 i, p_info->cid[i]);
            }
        }
        break;
...
T_APP_RESULT app_gfps_psm_msg_coc_conn_callback(uint8_t coc_type, void *p_coc_data)
{
    ...
    APP_PRINT_TRACE2("app_gfps_psm_msg_coc_conn_callback: coc_type = %d, bud_role %d",
                     coc_type, app_cfg_nv.bud_role);
    ...
    case GAP_COC_MSG_LE_CHANN_STATE:
        {
            ...
            p_link = app_gfps_link_find_le_link_by_conn_id(cb_data.p_le_chann_state->conn_id);
    ...
    case GAP_COC_MSG_LE_SEND_DATA:
        APP_PRINT_TRACE4("app_gfps_psm_msg_coc_conn_callback: GAP_COC_MSG_LE_SEND_DATA conn_id %d, cid 0x%x, cause 0x%x, credit %d",
...
//                       &gfps_eatt_le_proto_id, GAP_ECFC_DATA_PATH_GATT);
    ...
    APP_PRINT_TRACE0("app_gfps_psm_init");
}
#endif
