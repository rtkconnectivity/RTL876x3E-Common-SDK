...
{
    APP_PRINT_TRACE1("app_dult_handle_none_owner_state: new state %d", state);
...
static void app_dult_handle_info_opcode(T_SERVER_ID service_id, T_DULT_CALLBACK_DATA *p_callback)
{
    ...
    APP_PRINT_TRACE2("app_dult_handle_info_opcode: opcode %d, len %d", opcode, len);
    ...
    case DULT_OPCODE_INFO_GET_ACCESSORY_CAP:
        {
            ...
            APP_PRINT_TRACE1("DULT_OPCODE_INFO_GET_ACCESSORY_CAP: %d", rpt.accessory_cap);
            app_dult_send_indication(conn_handle, cid, service_id, (uint8_t *)&rpt, sizeof(rpt));
        }
        break;
    ...
    default:
        {
            APP_PRINT_TRACE1("app_dult_handle_ctrl_opcode: unhandled opcode %d", opcode);
        }
        break;
    }
}
...
static void app_dult_handle_ctrl_opcode(T_SERVER_ID service_id, T_DULT_CALLBACK_DATA *p_callback)
{
    ...
    APP_PRINT_TRACE2("app_dult_handle_ctrl_opcode: opcode 0x%x, len %d", opcode, len);
...
static T_APP_RESULT app_dult_cb(T_SERVER_ID service_id, void *p_data)
{
    ...
    APP_PRINT_TRACE1("app_dult_cb: msg_type %d", p_callback->msg_type);
