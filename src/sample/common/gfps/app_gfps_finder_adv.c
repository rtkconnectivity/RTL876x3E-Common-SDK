...
T_GAP_CAUSE gfps_finder_adv_update_adv_interval(uint32_t adv_interval)
{
    ...
    APP_PRINT_TRACE2("gfps_finder_adv_update_adv_interval: result %d, adv_interval %d",
                     result, adv_interval);
    return result;
}
...
static void gfps_finder_adv_state_change(T_BLE_EXT_ADV_CB_DATA cb_data)
{
    ...
    APP_PRINT_TRACE4("gfps_finder_adv_state_change: adv_state %d, adv_handle %d, stack stop cause 0x%x, app stop cause 0x%02x",
                     adv_state, adv_handle, stack_cause, app_cause);
...
void app_gfps_finder_adv_handle_mmi_action(void)
{
    if (extend_app_cfg_const.gfps_finder_support)
    {
        ...
        APP_PRINT_TRACE3("app_gfps_finder_adv_handle_mmi_action: bud_role %d, finder_conn_id %d, provisioned %d",
...
void gfps_finder_adv_init(uint8_t *p_adv_ei, uint8_t hash)
{
    ...
    {
        APP_PRINT_TRACE1("gfps_finder_adv_init: adv_handle %d", gfps_finder_adv_handle);
    }
}
#endif
