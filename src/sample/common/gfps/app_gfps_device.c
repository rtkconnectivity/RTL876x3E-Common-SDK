...
extern bool app_gfps_get_application_stop_adv_state(void);
...
void app_gfps_device_check_state(T_REMOTE_SESSION_ROLE device_role)
{
    if (!gfps_db.force_enter_pair_mode && (app_gfps_get_application_stop_adv_state() == true))
    {
        ...
        APP_PRINT_ERROR0("app_gfps_device_check_state: gfps adv is not allowed by application"
    ...
    if (app_db.device_state == APP_DEVICE_STATE_ON)
    {
        ...
        APP_PRINT_TRACE3("app_gfps_device_check_state: radio_mode %d, bud_role %d, b2s link %d",
                         radio_mode, device_role, app_link_get_b2s_link_num());
...
void app_gfps_device_handle_power_off(void)
{
    ...
    APP_PRINT_TRACE1("app_gfps_device_handle_power_off: gfps_support %d",
...
void app_gfps_device_handle_power_on(bool is_pairing)
{
    ...
    APP_PRINT_TRACE1("app_gfps_device_handle_power_on: gfps_support %d",
                     extend_app_cfg_const.gfps_support);
}
...
void app_gfps_device_handle_remote_conn_cmpl(void)
{
    ...
    if (app_cfg_nv.bud_role != REMOTE_SESSION_ROLE_SECONDARY)
    {
        APP_PRINT_TRACE0("app_gfps_device_handle_remote_conn_cmpl: pri sync account key and name");
#if F_GFPS_COMMON_FINDER_SUPPORT
        if (extend_app_cfg_const.gfps_finder_support)
        {
...
void app_gfps_device_handle_ble_link_disconnected(uint8_t local_disc_cause)
{
    ...
    if (extend_app_cfg_const.gfps_le_device_support)
    {
        if (local_disc_cause != GFPS_LE_LOCAL_DISC_CAUSE_GFPS_LINK_FULL)
        {
...
void app_gfps_device_handle_factory_reset(void)
{
    ...
    APP_PRINT_TRACE1("app_gfps_device_handle_factory_reset: gfps_support %d",
                     extend_app_cfg_const.gfps_support);
}
#endif
