    ...
    case DULT_CHAR_NONE_OWNER_CCCD_INDEX:
        {
            ...
            {
                APP_PRINT_TRACE0("dult_cccd_update_cb: enabled");
            }
            else
            {
                APP_PRINT_TRACE0("dult_cccd_update_cb: disabled");
            }
        }
        break;
...
{
    APP_PRINT_TRACE0("app_dult_send_indication");
                             ...
                             uint16_t opcode, T_DULT_CMD_RSP_STATE state)
{
    ...
    rpt.state = state;
    APP_PRINT_TRACE2("app_dult_report_cmd_rsp: opcode 0x%x, rsp_state %d", opcode, state);
