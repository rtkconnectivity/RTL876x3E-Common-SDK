...
void app_gfps_finder_generate_hash_flag(uint8_t *r, uint8_t len, uint8_t *hash)
{
    ...
    APP_PRINT_TRACE2("app_gfps_finder_generate_hash_flag: app_db.local_batt_level %d%, hash_flag.battery_level %d",
    ...
    *hash = input ^ output[31];
    APP_PRINT_TRACE4("app_gfps_finder_generate_hash_flag: sha256 input %b, sha256 output %b, hash input 0x%x, hash result 0x%x",
...
void app_gfps_finder_start_update_random_window_timer()
{
    ...
    APP_PRINT_TRACE1("app_gfps_finder_start_update_random_window_timer: random_value %d",
                     random_value);
...
void app_gfps_finder_handle_power_off(void)
{
    ...
    if (p_app_gfps_finder->gfps_finder_beacon_conn_id != 0xFF)
    {
        ...
        p_link = app_gfps_link_find_le_link_by_conn_id(p_app_gfps_finder->gfps_finder_beacon_conn_id);

        if (p_link != NULL)
        {
            app_gfps_ble_disconnect(p_link, GFPS_LE_LOCAL_DISC_CAUSE_GFPS_FINDER);
        }
    }
...
void app_gfps_finder_enter_beacon_state(T_GFPS_FINDER_BEACON_STATE beacon_state)
{
    ...
    APP_PRINT_TRACE3("app_gfps_finder_enter_beacon_state %d, eik.valid %d, adv_state %d",
                     beacon_state, p_app_gfps_finder->p_finder->eik.valid, adv_state);
...
bool app_gfps_finder_send_ring_rsp(uint8_t ring_state)
{
    ...
    if ((p_app_gfps_finder->gfps_finder_conn_id == 0xFF) ||
        (p_app_gfps_finder->gfps_finder_service_id == 0xFF))
    {
        APP_PRINT_TRACE0("app_gfps_finder_send_ring_rsp: invalid conn");
        return false;
    }
...
{
    APP_PRINT_TRACE3("app_gfps_finder_reset_conn: temp_modified_eik.valid %d, device_state %d, provisioned %d",
                     p_app_gfps_finder->p_finder->temp_modified_eik.valid,
                     app_db.device_state, app_gfps_finder_provisioned());
    ...
    if (p_app_gfps_finder->p_finder->temp_modified_eik.valid == true)
    {
        ...
        ftl_save_to_storage(&p_app_gfps_finder->p_finder->eik, GFPS_FINDER_EIK_FLASH_OFFSET,
                            sizeof(T_GFPS_EIK));
        //if EIK update, we shall stop finder adv and reopen it again to make sure using the correct EIK
        gfps_finder_adv_stop(APP_STOP_ADV_CAUSE_GFPS_FINDER_UPDATE_EIK);
    }

    //in power off state, when ble link disconnected, need enter power down mode
    ...
    {
        //in power on state, when ble link disconnected, need restart finder adv
...
void app_gfps_finder_send_eddystone_capability(uint8_t *bd_addr, uint8_t local_server_chann)
{
    ...
    APP_PRINT_TRACE1("app_gfps_finder_send_eddystone_capability: ret %d", ret);
}
...
void app_gfps_finder_update_clock_value(uint32_t clock_value_delayed)
{
    if (p_app_gfps_finder->p_finder->eik.valid)
    {
        ...
        APP_PRINT_TRACE2("app_gfps_finder_update_clock_value: start finder adv, old clock %d, new clock %d",
...
void app_gfps_finder_update_hash(void)
{
    ...
    APP_PRINT_TRACE2("app_gfps_finder_update_hash: hash 0x%x, hash_flag 0x%x", hash, hash_flag);
}
...
void app_gfps_finder_update_battery(uint8_t local_battery)
{
    ...
    APP_PRINT_TRACE3("app_gfps_finder_update_battery: local_battery %d%, old_battery_level %d, new_battery_level %d",
...
T_GFPS_FINDER_CAUSE app_gfps_finder_cb(T_GFPS_FINDER_CB_DATA *p_data)
{
    ...
    uint8_t ret_err = 0;
    APP_PRINT_TRACE1("app_gfps_finder_cb: evt %d", evt);
    ...
    case GFPS_FINDER_EVT_SET_EIK:
        {
            ...
            APP_PRINT_TRACE2("app_gfps_finder_cb: load eik %b, valid %d",
...
void app_gfps_finder_ftl_init(void)
{
    ...
    APP_PRINT_TRACE4("app_gfps_finder_ftl_init: eik %b, valid %d, eik size %d, clock %d",
...
void app_gfps_finder_handle_mmi_action_stop_ring(void)
{
    if (extend_app_cfg_const.gfps_finder_support)
    {
    ...
    APP_PRINT_TRACE1("app_gfps_finder_handle_mmi_action_stop_ring: finder_support %d",
                     extend_app_cfg_const.gfps_finder_support);
}


uint32_t app_gfps_finder_set_wakeup_time(void)
{
#if F_GFPS_COMMON_PERIODIC_WAKEUP
    ...
    if (extend_app_cfg_const.gfps_finder_support &&
        ...
        app_gfps_finder_provisioned())
    {
        wakeup_time = extend_app_cfg_const.gfps_power_off_start_finder_adv_timer_timeout_value;
    }

    APP_PRINT_TRACE1("app_gfps_finder_set_wakeup_time: %d", wakeup_time);
    return wakeup_time;
#else
    APP_PRINT_ERROR0("app_gfps_finder_set_wakeup_time: not support");
    return 0;
#endif
}
