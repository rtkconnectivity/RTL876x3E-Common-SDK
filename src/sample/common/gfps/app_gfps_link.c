...
void app_gfps_link_print_link_priority()
{
    ...
    for (uint8_t i = 0; i < gfps_link_priority_q.count; i++)
    {
        ...
        APP_PRINT_TRACE2("app_gfps_print_link_priority: conn_id %d, priority %d", op->conn_id, i);
    }
}

void app_gfps_link_add_link_into_priority_queue(uint8_t conn_id)
{
    APP_PRINT_TRACE1("app_gfps_link_add_link_into_priority_queue: conn_id %d", conn_id);
...
uint8_t app_gfps_link_find_gfps_link(void)
{
    ...
    APP_PRINT_TRACE1("app_gfps_link_find_gfps_link: link_num %d", link_num);
    return link_num;
}
...
void app_gfps_link_disconnect_all_gfps_link(void)
{
    for (uint8_t i = 0; i < MAX_BLE_LINK_NUM; i++)
    {
        ...
        {
            APP_PRINT_TRACE1("app_gfps_link_disconnect_all_gfps_link: gfps_conn_id %d",
                             le_link->gfps_link.gfps_conn_id);
            app_gfps_ble_disconnect(le_link, GFPS_LE_LOCAL_DISC_CAUSE_GFPS_STOP);
