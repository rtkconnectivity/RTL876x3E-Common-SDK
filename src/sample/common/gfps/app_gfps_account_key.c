...
bool app_gfps_account_key_init(uint8_t key_num)
{
    ...
    if (account_key)
    {
        ...
        else
        {
            ...
            {
                APP_PRINT_TRACE4("app_gfps_account_key_init: idx %d, key %b, bd_addr %b, del %d", i,
    ...
    {
        APP_PRINT_TRACE0("app_gfps_account_key_init: alloc account key memory fail");
        return false;
    }
    ...
    for (uint8_t i = 0; i < account_key->num; i++)
    {
        if (memcmp(bd_addr, account_key->account_info[i].addr, 6) == 0)
        {
            if (memcmp(key, account_key->account_info[i].key, 16) == 0)
            {
                ...
                APP_PRINT_TRACE1("app_gfps_account_key_store: key exist i %d", i);
                return false;
            }
            else
            {
                ...
                APP_PRINT_TRACE3("app_gfps_account_key_store: bd_addr %b already exist, key update index i %d, key %b",
...
{
    APP_PRINT_TRACE0("app_gfps_account_key_clear");
    memset(account_key, 0, gfps_account_key_table_size);
...
{
    APP_PRINT_TRACE0("app_gfps_account_key_remote_add");
...
{
    APP_PRINT_TRACE0("app_gfps_account_key_remote_sync");
...
void app_gfps_account_key_table_print(void)
{
    for (uint8_t i = 0; i < account_key->num;  i++)
    {
        APP_PRINT_TRACE3("app_gfps_account_key_table_print: idx %d, key %b, bd_addr %b", i,
                         TRACE_BINARY(16, account_key->account_info[i].key),
...
bool app_gfps_account_key_find_index_by_account_key(uint8_t *index, uint8_t *target_account_key)
{
    ...
    APP_PRINT_TRACE2("app_gfps_account_key_find_index_by_account_key ret %d, accountkey %b, index %d",
                     ret, TRACE_BINARY(16, account_key), *index);
    return ret;
}
                                     ...
                                     uint8_t *msg_nonce, uint8_t *mac)
{
    ...
    APP_PRINT_TRACE1("app_gfps_account_key_verify_mac %d %d, %b", ret, inuse_account_key_index,
                                                 ...
                                                 uint8_t *msg, uint16_t data_len, uint8_t *msg_nonce, uint8_t *mac)
{
    ...
    APP_PRINT_TRACE6("app_gfps_account_key_find_inuse_account_key id %d, addr %s, bd_addr %s, ret %d index %d, account key %b",
                     conn_id, TRACE_BDADDR(addr), TRACE_BDADDR(bd_addr), ret,
...
{
    APP_PRINT_TRACE0("app_gfps_account_key_remote_handle_add: sec store account key");
    ...
    for (uint8_t i = 0; i < p_table->num; i++)
    {
        if (memcmp(bd_addr, p_table->account_info[i].addr, 6) == 0)
        {
            ...
            else
            {
                ...
                APP_PRINT_TRACE1("app_gfps_account_key_add: key update i %d", i);
...
{
    APP_PRINT_TRACE0("app_gfps_account_key_remote_handle_sync: sec store account key");
...
{
    APP_PRINT_TRACE1("app_gfps_account_key_save_ownerkey_valid: valid %d",
                     account_key->owner_key_valid);
    ftl_save_to_storage(account_key, ACCOUNT_KEY_FLASH_OFFSET, gfps_account_key_table_size);
}
#endif
#endif
