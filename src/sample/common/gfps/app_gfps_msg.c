...
void app_gfps_msg_update_ring_status(uint8_t ring_status)
{
    ...
    for (i = 0; i < MAX_BR_LINK_NUM; i++)
    {
            ...
            (app_db.br_link[i].connected_profile & GFPS_PROFILE_MASK))
        {
            ...
            APP_PRINT_TRACE1("app_gfps_msg_update_ring_status: path br, ring_status %d", ring_status);
        }
    }
    ...
    for (i = 0; i < MAX_BLE_LINK_NUM; i++)
    {
        if ((app_gfps_link_db.le_link[i].used == true) &&
            (app_gfps_link_db.le_link[i].gfps_link.gfps_conn_id != 0xFF) &&
            (app_gfps_link_db.le_link[i].gfps_link.gfps_msg_cid != 0))
        {
            T_APP_GFPS_LE_LINK *p_link = &app_gfps_link_db.le_link[i];

            if (p_link != NULL)
            {
                bt_gfps_send_ring_status(app_gfps_link_db.le_link[i].gfps_link.gfps_conn_id,
                                         app_gfps_link_db.le_link[i].gfps_link.gfps_msg_cid,
                                         NULL, 0, &ring_status, 1);
            }

            APP_PRINT_TRACE1("app_gfps_msg_update_ring_status: path BLE, ring_status %d", ring_status);
        }
    }
}
...
{
    app_audio_tone_type_play(app_gfps_cfg_get_tone_type(), false, false);
...
void app_gfps_msg_ring_stop(void)
{
    ...
    app_audio_tone_type_cancel(app_gfps_cfg_get_tone_type(), false);
...
#endif
    APP_PRINT_TRACE2("app_gfps_msg_handle_ring_event: state 0x%02x, event 0x%02x",
                                       ...
                                       uint8_t *p_cmd, uint16_t cmd_len)
{
    ...
    while (len >= GFPS_HEADER_LEN)
    {
            ...
            case GFPS_ACITVE_COMPONENTS_REQ:
                {
                    ...
                    APP_PRINT_TRACE1("gfps active_components %d", active_components);
        ...
        if (unknow_cmd == true)
        {
            APP_PRINT_TRACE0("app_gfps_msg_handle_data_transfer: unknow cmd");
            break;
        }
    }
}
...
void app_gfps_msg_le_loop_check_data_complete(uint8_t conn_id, uint16_t cid, uint8_t *data,
                                              uint16_t len)
{
    APP_PRINT_TRACE3("app_gfps_msg_le_loop_check_data_complete: conn_id %d, cid 0x%x, data %b",
    ...
    if (p_link != NULL)
    {
        ...
        APP_PRINT_TRACE4("app_gfps_msg_le_loop_check_data_complete: len %d, cmd_len %d, ret %d, cont %d\n",
                         len, p_link->gfps_link.gfps_cmd_len, ret, cont);
        if (ret)
        {
...
static void app_gfps_msg_rfc_bt_cback(T_BT_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    case BT_EVENT_REMOTE_DISCONN_CMPL:
        {
            ...
            APP_PRINT_TRACE3("app_gfps_msg_rfc_bt_cback: gfps_msg_ring_state 0x%02x, bud_role %d, bud_side %d",
    ...
    if (handle == true)
    {
        APP_PRINT_TRACE1("app_gfps_msg_rfc_bt_cback: event_type 0x%04x", event_type);
    }
}
...
static void app_gfps_msg_rfc_audio_cback(T_AUDIO_EVENT event_type, void *event_buf,
                                         uint16_t buf_len)
{
    APP_PRINT_TRACE1("app_gfps_msg_rfc_audio_cback: event_type 0x%04x", event_type);
    bool handle = true;
    ...
    case AUDIO_EVENT_VOICE_PROMPT_STARTED:
        {
            ...
            if (tone_started_idx == app_gfps_cfg_get_tone_idx())
            {
#if F_GFPS_COMMON_FINDER_SUPPORT
                if (extend_app_cfg_const.gfps_finder_support)
                {
    ...
    case AUDIO_EVENT_VOICE_PROMPT_STOPPED:
        {
            ...
            APP_PRINT_TRACE2("app_gfps_msg_rfc_audio_cback: tone_stopped_idx %d, app_gfps_cfg_get_tone_idx() %d",
                             tone_stopped_idx, app_gfps_cfg_get_tone_idx());

            if (tone_stopped_idx == app_gfps_cfg_get_tone_idx())
            {
#if F_GFPS_COMMON_FINDER_SUPPORT
                if (extend_app_cfg_const.gfps_finder_support)
                {
...
{
    bool skip = true;
    APP_PRINT_TRACE1("app_gfps_msg_relay_cback: msg_type %d", msg_type);
    ...
    if (buf != NULL)
    {
        ...
        APP_PRINT_TRACE1("app_gfps_msg_relay_cback: buf %b", TRACE_BINARY(msg_len, buf));
    }
...
void app_gfps_msg_update_rpa_addr(void)
{
    ...
    for (i = 0; i < MAX_BR_LINK_NUM; i++)
    {
            ...
            (app_db.br_link[i].connected_profile & GFPS_PROFILE_MASK))
        {
            ...
            APP_PRINT_TRACE1("app_gfps_rfc_update_ble_addr: BR link_id %d", i);
        }
    }
    ...
    for (i = 0; i < MAX_BLE_LINK_NUM; i++)
    {
            ...
            (app_gfps_link_db.le_link[i].gfps_link.gfps_msg_cid != 0))
        {
            ...
            if (p_link != NULL)
            {
                bt_gfps_send_ble_addr(app_gfps_link_db.le_link[i].gfps_link.gfps_conn_id,
                                      app_gfps_link_db.le_link[i].gfps_link.gfps_msg_cid,
                                      NULL, 0, ble_addr);
            }

            APP_PRINT_TRACE1("app_gfps_rfc_update_ble_addr: BLE link_id %d", i);
        }
    }
}

void app_gfps_msg_audio_and_rfc_init(void)
{
