...
#if (F_GFPS_COMMON_BASIC_FEATURE_SUPPORT && F_GFPS_COMMON_BATTERY_INFO_REPORT)
...
void app_gfps_battery_set_info(T_GFPS_BATTERY_INFO *p_battery_info)
{
    ...
    if (app_cfg_nv.bud_role != REMOTE_SESSION_ROLE_SECONDARY)
    {
        ...
        if (gfps_db.gfps_curr_action == GFPS_ACTION_ADV_NOT_DISCOVERABLE_MODE)
        {
            ...
            APP_PRINT_TRACE0("app_gfps_battery_set_info: updated battery by LE,popup show ui");
        }
        else if (gfps_db.gfps_curr_action == GFPS_ACTION_ADV_NOT_DISCOVERABLE_MODE_HIDE_UI)
        {
            ...
            APP_PRINT_TRACE0("app_gfps_battery_set_info: updated battery by LE,popup hide ui");
        }
    }
    ...
    for (i = 0; i < MAX_BR_LINK_NUM; i++)
    {
            ...
            (app_db.br_link[i].connected_profile & GFPS_PROFILE_MASK))
        {
            ...
            bt_gfps_updated_battery(0xFF, 0, p_link->bd_addr, p_link->gfps_rfc_chann, battery,
                                    GFPS_BATTERY_LEN);
            APP_PRINT_TRACE3("app_gfps_battery_rfc_update: left 0x%x, right 0x%x, case 0x%x", battery[0],
                             battery[1], battery[2]);
        }
    }
}
...
{
    for (uint8_t i = 0; i < APP_GFPS_MAX_LINKS; i++)
    {
            ...
            (le_link->gfps_link.gfps_conn_state == GAP_CONN_STATE_CONNECTED))
        {
            ...
            APP_PRINT_TRACE4("app_gfps_battery_le_l2cap_update: conn_id %d, left 0x%x, right 0x%x, case 0x%x",
...
void app_gfps_battery_info_report(T_GFPS_BATTERY_REPORT_EVENT event)
{
    ...
    APP_PRINT_TRACE1("app_gfps_battery_info_report: event %d", event);
...
#else
    APP_PRINT_TRACE0("app_gfps_battery_info_report: not support");
#endif
}
...
{
    APP_PRINT_TRACE1("app_gfps_battery_show_ui: %d", show_ui);
    T_GFPS_BATTERY_INFO battery_info;
    battery_info = app_gfps_battery_get_info();
